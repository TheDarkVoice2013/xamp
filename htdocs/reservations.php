...
$(document).ready(function() {
    $('#searchBtn').on('click', searchReservations);
    function searchReservations() {
        $.ajax({
            url: 'get_reservations_user.php',
            method: 'POST',
            data: {},
            success: function(response) {
                var results = JSON.parse(response).results;
                var tableBody = $('#hotel-table-body');

                tableBody.empty();

                if (results.length > 0) {
                    for (var result of results) {
                        var numeHotel = result.name;
                        var occupancy = result.occupancy_limit;
                        var startDate = result.start_date;
                        var endDate   = result.end_date;
                        var roomType  = result.type;
                        ...
                        row += '<td>' + roomType + '</td>';
                        row += '<td>' + status + '</td>';
                        row += '<td><button class="action-button" data-id="' + id + '">Cancel</button></td>';
                        row += '</tr>';
                        tableBody.append(row);
                    }
                } else {
                    tableBody.append('<tr><td colspan="4" class="no-hotels-found">No hotels found</td></tr>');
                }
            },
            error: function(error) {
                console.error('Error fetching reservations:', error);
                $('#hotel-table-body').append('<tr><td colspan="4" class="no-hotels-found">Error fetching hotels data</td></tr>');
            }
        });
    };
  
    $('#hotel-table-body').on('click', '.action-button', function() {
        var id = $(this).data('id');
